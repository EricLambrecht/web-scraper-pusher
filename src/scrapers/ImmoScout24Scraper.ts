...
export default class ImmoScout24Scraper extends ListChangeScraper {
    ...
    async handleCookieBanner() {
        try {
            const cookieBanner = await this.page.waitForSelector('app-notice', { timeout: 5000 })
            if (cookieBanner) {
                const acceptButton = await this.page.$('app-notice .action-buttons button:last-child')
